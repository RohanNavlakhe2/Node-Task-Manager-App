...
router.post('/users', async (req, res) => {
    ...
    try {
        sendEmail({
            receipentEmail:newUser.email,
            subject:'Welcome to Task Manager app',
            body:'Manage and access your tasks anywhere!'
        })
...
router.delete('/users/me', authMiddleware, async (req, res) => {
    try {
        ...
        sendEmail({
            receipentEmail:req.user.email,
            subject:'Your account has been deleted with Task Manager app',
            body:'Thanks for using our app'
        })

    } catch (e) {
        log('err in delete user : ',e)
        res.status(400).send({message: e})
    }
})
